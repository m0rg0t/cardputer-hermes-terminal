...
word wrap at 38 to 39 columns, counted in code points. The 6 x 8 font carries
ASCII glyphs only, so Cyrillic (U+0410..U+044F, Ё/ё) is drawn by a built-in
5 x 7 glyph set at the same 6 px advance (`include/hermes_terminal/cyrillic_font.h`,
regenerated by `scripts/generate_cyrillic_font.py`). Other scripts fold to
`?`. Build with `-DHERMES_CYRILLIC_FONT=0` to drop the glyphs (about 1.5 KB).
